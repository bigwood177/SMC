;DUCPRC.COP
;
;		AUTHOR: SHERWOOD S. QUIRING
;		DATE  : 18-DEC-1996
;
RECORD	COPTBL
	.INCLUDE 'DEF:RD182A.DEF'

RECORD	PARAMS
	.INCLUDE 'DEF:PARAM.DEF'

RECORD	DPKEY
	K_CODE	,A2
	K_GAUGE	,D3
	K_TYPE	,D1
	K_CAT	,D1
	K_STYLE	,D1

RECORD	CHANNEL
	CHN182	,D2

RECORD	CTL
	ROW	,D2
		,A1
	COL	,D2
		,A1
	MAX	,D2
		,A1
	MIN	,D2
		,A1
	TYPE	,A2

RECORD	VARS
	I	,D5
	J	,D5
	SELECT	,D1
	OPNOK	,D1
	ENTRY	,A30
	INXCTL	,D1
	WHATNO	,D2
	CNGCTL	,D1
	READ	,D1,0
	WRITE	,D1,1
	STORE	,D1,2
	DELETE	,D1,3
	LOKCTL	,D1
	SWITCH	,D1
	V	,D1

PROC
	XCALL TERID (V)
	V = 1

	XCALL OUTPT (1,1,2,'DUCT PRICING TABLE',1)
	CALL OPENS
	IF (.NOT. OPNOK)
		BEGIN
		CALL CLOSE
		XCALL PGCHN ('CP:CPMENU',1)
		END

DISPLA,
	CLEAR CNGCTL
	XCALL OUTPT (1,1,2,'DUCT PRICING TABLE',1)
	XCALL OUTPT ( 4,4,0,'1. GAUGE',1)
	XCALL OUTPT ( 6,4,0,'2. TYPE',1)
	XCALL OUTPT ( 8,4,0,'3. CAT',1)
	XCALL OUTPT (10,4,0,'4. STYLE',1)
	XCALL OUTPT (12,4,0,'5. PRICE',1)
DP_GAUGE,
	CTL = '04,13,03,00,#E'
	CALL INPUT
	GOTO (DISPLA,ENDOFF),INXCTL
	K_GAUGE = ENTRY(1,3)
	IF (K_GAUGE .EQ. 0)
	  BEGIN
	  XCALL IOS (CHN182,COPTBL,READ,LOKCTL)	;READ NEXT
	  IF (LOKCTL .EQ. 0 .AND. TBLCOD .EQ. 'DP')
		BEGIN
		SELECT = 2		;EXISTING RECORD
		K_GAUGE = DP_GAUGE
		K_TYPE = DP_TYPE
		K_CAT = DP_CAT
		K_STYLE = DP_STYLE
		CALL DSPREC
		GOTO ANYCNG
		END
	  END
	IF (K_GAUGE.NE.18 .AND.
&	    K_GAUGE.NE.20 .AND.
&	    K_GAUGE.NE.22 .AND.
&	    K_GAUGE.NE.24 .AND.
&	    K_GAUGE.NE.26) GOTO DP_GAUGE
	GOTO (ANYCNG),CNGCTL

DP_TYPE,
	CTL = '06,13,01,00,# '
	FOR I FROM 1 THRU 4 XCALL OUTPT (ROW+I-1,65,0,DTYPE(I),V)
	CALL INPUT

	GOTO (DISPLA),INXCTL
	K_TYPE = ENTRY(1,1)
	IF (K_TYPE .EQ. 0)
		BEGIN
		K_TYPE = 1
		XCALL OUTPT (6,13,0,'1',1)
		END
	FOR I FROM 1 THRU 4 XCALL OUTPT (ROW+I-1,65,1,'\',V)
	CLEAR ENTRY
	ENTRY(1,1) = K_TYPE,	'X'
	ENTRY(5,19) = DTYPE(K_TYPE)
	CLEAR ENTRY(5,7)
	XCALL OUTPT (ROW,13,0,ENTRY,1)
	GOTO (ANYCNG),CNGCTL
DP_CAT,
	CTL = '08,13,01,00,# '
	FOR I FROM 1 THRU 4 XCALL OUTPT (ROW+I-1,65,0,DCAT(I),V)
	CALL INPUT
	GOTO (DISPLA), INXCTL
	K_CAT = ENTRY(1,1)
	IF (K_CAT.EQ.0)K_CAT = 1
	IF (K_CAT.LT.1.OR.K_CAT.GT.4) GOTO DP_CAT
	CLEAR ENTRY
	ENTRY(1,1) = K_CAT,'X'
	ENTRY (5,19) = DCAT(K_CAT)
	FOR I FROM 1 THRU 4 XCALL OUTPT (ROW+I-1,65,1,'\',V)
	CLEAR ENTRY(5,7)
	XCALL OUTPT (ROW,13,0,ENTRY,1)

	GOTO (ANYCNG),CNGCTL
DP_STYLE,
	CTL = '10,13,01,00,# '
	FOR I FROM 1 THRU 5	XCALL OUTPT (ROW+I-1,65,0,DSTY(I),V)
	CALL INPUT
	GOTO (DISPLA),INXCTL
	K_STYLE = ENTRY(1,1)

	IF (K_STYLE.EQ.0) K_STYLE = 1
	IF (K_STYLE.LT.1.OR.K_STYLE.GT.5) GOTO DP_STYLE
	CLEAR ENTRY
	ENTRY(1,1) = K_STYLE,'X'
	ENTRY (5,19) = DSTY(K_STYLE)
	CLEAR ENTRY(5,7)
	FOR I FROM 1 THRU 5 XCALL OUTPT (ROW+I-1,65,1,'\',V)
	XCALL OUTPT (ROW,13,0,ENTRY,1)

	K_CODE = 'DP'
	XCALL ISIO (CHN182,COPTBL,DPKEY,READ,LOKCTL)
	IF (LOKCTL .EQ. 0)
	THEN	BEGIN
		SELECT = 2		;EXISTING RECORD
		K_GAUGE = DP_GAUGE
		K_TYPE = DP_TYPE
		K_CAT = DP_CAT
		K_STYLE = DP_STYLE
		CALL DSPREC
		GOTO ANYCNG
		END
	ELSE	BEGIN
		SELECT = 1
		END
	GOTO (ANYCNG),CNGCTL

DP_PRICE,
	CTL = '12,13,08,00,# '
	CALL INPUT
	GOTO (DISPLA),INXCTL
	DP_PRICE = ENTRY(1,8)
	ENTRY(1,10) = DP_PRICE,	'ZZ,ZZX.XXX' [LEFT]
	XCALL OUTPT (ROW,13,0,ENTRY(1,9),1)
	GOTO (ANYCNG),CNGCTL
ANYCNG,
	XCALL ANYCN(CNGCTL,WHATNO)
	GOTO (PROCES,CNGBR),CNGCTL + 1
CNGBR,
	CASE SELECT OF
	BEGINCASE
	1:	GOTO (DP_GAUGE,DP_TYPE,DP_CAT,DP_STYLE,DP_PRICE),WHATNO
	2:	GOTO (ANYCNG,ANYCNG,ANYCNG,ANYCNG,DP_PRICE),WHATNO
	ENDCASE

	GOTO ANYCNG
PROCES,
	TBLCOD = 'DP'	
	DP_GAUGE = K_GAUGE
	DP_TYPE = K_TYPE
	DP_CAT = K_CAT
	DP_STYLE = K_STYLE

	CASE SELECT OF
	BEGINCASE
	1:	XCALL ISIO (CHN182,COPTBL,TBL_KEY,STORE,LOKCTL)
	2:	XCALL ISIO (CHN182,COPTBL,TBL_KEY,WRITE,LOKCTL)
	3:	BEGIN
		XCALL ISIO (CHN182,COPTBL,TBL_KEY,DELETE,LOKCTL)
		XCALL MESAG ('RECORD DELETED !',1)
		END
	ENDCASE
	GOTO DISPLA

DSPREC,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	CLEAR ENTRY
	ENTRY(1,3) = DP_GAUGE,	'ZZX' [LEFT]
	XCALL OUTPT (4,13,0,ENTRY(1,3),1)
	
	CLEAR ENTRY
	ENTRY(1,1) = DP_TYPE,	'X'
	ENTRY(5,19) = DTYPE(DP_TYPE)
	CLEAR ENTRY(5,7)
	XCALL OUTPT (6,13,0,ENTRY,1)

	CLEAR ENTRY
	ENTRY(1,1) = DP_CAT,'X'
	ENTRY (5,19) = DCAT(DP_CAT)
	CLEAR ENTRY(5,7)
	XCALL OUTPT (8,13,0,ENTRY,1)

	CLEAR ENTRY
	ENTRY(1,1) = DP_STYLE,'X'
	ENTRY (5,19) = DSTY(DP_STYLE)
	CLEAR ENTRY(5,7)
	XCALL OUTPT (10,13,0,ENTRY,1)

	CLEAR ENTRY
	ENTRY(1,10) = DP_PRICE,	'ZZ,ZZX.XXX' [LEFT]
	XCALL OUTPT (12,13,0,ENTRY(1,10),1)

	RETURN
;--------------------------------------------------------

INPUT,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	XCALL INPUT(ROW,COL,MAX,MIN,TYPE,ENTRY,INXCTL,1)
	RETURN
;--------------------------------------------------------

ENDOFF,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	CALL CLOSE
	XCALL PGCHN ('CP:TBLMNU',1)
;------------------------------------

OPENS,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	CLEAR OPNOK

	SWITCH = 5
	XCALL FILES (1,'SU',182,SWITCH)		;182 - COPTBL
	IF (SWITCH .EQ. 9) RETURN
	CHN182 = 1
	OPNOK = 1
	RETURN
;------------------------------------

CLOSE,	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	IF (CHN182) CLOSE CHN182
	RETURN
;------------------------------------

