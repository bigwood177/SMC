...
NXTREC,
	...
	IF (PRDSTR.EQ.']]]]]]]]') RETURN	;RETURN TO PSTINV, NO
						;MORE RECORDS TO PROCESS
	IF (PITMNO.LT.LITMNO) GOTO NXTREC
	IF (PITMNO.GT.LITMNO) RETURN		;RETURN TO PSTINV
...
BADMSG,
	IF (BADBIL.EQ.1) RETURN		;GO BACK TO PSTINV AND DON'T ALLOW
					;THIS LINE TO BE ORDERED
	XCALL MESAG ('ERROR ENCOUNTERED - READJUSTING INVENTORY.
&		 PLEASE WAIT.',4)
	BADBIL = 1
	RECNO = FSTREC
	GOTO NXTREC
...
NEWCMP,
	...
	QTYCTR =
	DECMAL = LQTYOR * 10000
	IF (LQTYSH.LT.LQTYOR.AND.BOCODE.NE.1) DECMAL = LQTYSH * 10000
				;IF QTY SHIPPED IS LESS
				;THAN QTY ORDERED, LEAVE BALANCE COMMITTED
				;IF ITEM IS BACKORDERABLE
CHAINQ,
	INCR QTYCTR
...
FNDSTK,
	...
	IF (STOKED.NE.'S'.OR.CNTRLD.NE.'C') GOTO NONSTK		;DO NOT ADJUST
				;QTY COMMITTED IF NON-STOCKED OR NON-CONTROLLED
	IF (BADBIL.EQ.1) DECMAL = (-1) * DECMAL
	IF (BADBIL.NE.1) QTYCOM(LOCCTR) = QTYCOM(LOCCTR) - DECMAL
;;;	IF (QTYCOM(LOCCTR).LT.0) QTYCOM(LOCCTR) =
NONSTK,
	IF (BADBIL.EQ.1) DECMAL = (-1) * DECMAL
	IF (STOKED.EQ.'S') QTYONH(LOCCTR) = QTYONH(LOCCTR) - DECMAL
	USEMTD = USEMTD + DECMAL	;UPDATE USAGE MTD
	USEYTD = USEYTD + DECMAL	;UPDATE USAGE YTD
	LOKCTL = 1
